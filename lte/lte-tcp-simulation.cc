...
main (int argc, char *argv[])
{
  ...
  std::string trafficScenario = "both";
  double x2Distance = 500.0;        // m
  double yDistanceForUe = 1000.0;   // m
  double buildingSize = 20.0;       // m
  double speed = 20;                // m/s
  double enbTxPowerDbm = 46.0;      // Dbm
  std::string handoverType = "A2A4"; // Handover Type String
  bool useRlcUm = false;
  bool verbose = false;
  bool pcap = false;
  ...
  std::string traceFilePrefix = "lte-tcp-simulation";
  ...
  cmd.AddValue ("buildingSize", "buildingSize value (meters) for building", buildingSize);
  ...
  cmd.AddValue ("trafficScenario","outdoor / indoor / both", trafficScenario);
  ...
  // Set Propagation Loss Model as BuildingPropagationModel
  ...
  //NodeContainer homeueNodes;
  NodeContainer enbNodes;
  enbNodes.Create (numberOfEnbs);
  ueNodes.Create (numberOfUes);
  //homeueNodes.Create(numberOfUes);

  // Create Building
  double x_min = x2Distance + 100.0;
  double x_max = x_min + buildingSize;
  double y_max = x2Distance - 380.0;
  double y_min = y_max - buildingSize;
  double z_min = 0.0;
  double z_max = 10.0;
  ...
  BuildingsHelper::Install (ueNodes.Get(0) );  
  ...
  BuildingsHelper::Install (ueNodes.Get(1));
  ...
  //NetDeviceContainer homeueLteDevs = lteHelper->InstallUeDevice (homeueNodes);
  ...
  if(trafficScenario == "both" || trafficScenario == "outdoor")
  {
  ...
  if(trafficScenario == "both" || trafficScenario == "indoor")
  {
    Address sinkLocalAddress_2 (InetSocketAddress (Ipv4Address::GetAny (), port));
    PacketSinkHelper sinkHelper2 ("ns3::TcpSocketFactory", sinkLocalAddress_2);
  ...
  if (pcap)
    {
      p2ph.EnablePcapAll ("lte-tcp-simulation");
    }
